...
class Generator
{
    ...
    private function createImage($srcImagePath)
    {
        $info = getimagesize($srcImagePath);
        if($info !== flase) {
            switch ($info['mime']) {
                case "image/gif":
                    $resource = @imagecreatefromgif($srcImagePath);
                    break;
                case "image/jpeg":
                    $resource = @imagecreatefromjpeg($srcImagePath);
                    break;
                case "image/png":
                    $resource = @imagecreatefrompng($srcImagePath);
                    break;
            }
        } else {
            $image = imagecreatefromstring(base64_decode($srcImagePath))
            if ($image !== false) {
                $resource = $this->createImage($image);
            }
        }
        return $resource;
    }
    ...
    {
        if (($srcImage = $this->createImage($srcImagePath)) === false) {
            return;
        }
